...
(async()=>{
  ...
  try{
    ...
    await page.goto(`http://127.0.0.1:${server.address().port}/hogoneko/`);
    ...
    const response=await page.request.get(`http://127.0.0.1:${server.address().port}/hogoneko/game.html`);assert.equal(response.status(),404);
